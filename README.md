...
- `--model {gemini,claude,gpt}`: Select which model's benchmark set to analyze
  - `gemini`: Benchmarks used in Gemini 3 Pro's launch evaluation
  - `claude`: Benchmarks used in Claude Opus 4.5's launch evaluation
  - `gpt`: Benchmarks used in GPT 5.2's launch evaluation
...
- **`BENCHMARK INFO.csv`**: Master list of 37 benchmarks with website/paper links
...
1. **Benchmark Selection**: Loads benchmarks from `BENCHMARK INFO.csv` based on `--model` flag
